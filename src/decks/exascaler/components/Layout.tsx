...
export default function Layout({ children }: LayoutProps) {
  ...
  const presentationContextValue: PresentationContextType = {
    isPresentationMode,
    currentSlide,
    totalSlides,
    goToSlide,
    ...
    <PresentationContext.Provider value={presentationContextValue}>
      <div className={`${styles.layout} ${isPresentationMode ? styles.presentationMode : ''}`}>
        ...
        {!isEmbedded && <nav className={`${styles.nav} ${scrolled ? styles.scrolled : ''} ${isPresentationMode ? styles.presentationNav : ''}`}>
          <div className={styles.navInner}>
            ...
            <Link to="/" className={styles.logo}>
              <img src="/new_logo-dark.png" alt="DDN" className={styles.logoImg} />
            </Link>

            {/* Center Title */}
            <span className={styles.pageTitle}>EXAScaler</span>
        ...
        <main
          ...
          style={isEmbedded && isPresentationMode ? { position: 'fixed', inset: 0, padding: 0, margin: 0, overflow: 'hidden', background: '#000' } : undefined}
        >
          {children}
        </main>
