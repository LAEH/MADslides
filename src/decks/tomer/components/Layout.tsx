...
import { slideMetadata } from '../slides'
...
// Slide names from registry
const SLIDE_NAMES = slideMetadata.map(s => s.title)
      ...
      <div className={`${styles.layout} ${isPresentationMode ? styles.presentationMode : ''}`}>
        ...
        {!isEmbedded && <nav className={`${styles.nav} ${scrolled ? styles.scrolled : ''} ${isPresentationMode ? styles.presentationNav : ''}`}>
          <div className={styles.navInner}>
            ...
            {/* Center Title */}
            <span className={styles.pageTitle}>DDN Infinia Technical Overview</span>
        ...
        <main
          ...
          style={isEmbedded && isPresentationMode ? { position: 'fixed', inset: 0, padding: 0, margin: 0, overflow: 'hidden', background: 'var(--surface-base)' } : undefined}
        >
          {children}
        </main>
